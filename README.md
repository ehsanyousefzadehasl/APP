...
### Final Project
In the final project of this course, a cool program implemented that scrapes information from bama.ir, then filling the database with them information. In this phase, the focus was on avoiding keeping the duplicate data, so primary key defined in fulfilling that goal. Then, used a tree model to train that and predict an input car's price. You can see the project's source code [here](code/06-final_project/final_project.py).